select count( *) from employees,departments ; --cross join 2889개
...
select employees a , departments d 
from department_id d , employee_id , salary , create_date , update_date , emp_ name , job_id ='David austin'
;
select department David austin;
...
;
select employee_id , emp_name , salary , department_id , job_id , department_name , b.create_date, b.update_date
from employees a, departments b
where a.department_id = b.dapartments b and
a.department_id =( select department_id from employees where emp_name = 'David Austin')
;
...
and a.department_id = (select department_id from employees where emp_name='David Austin') ;
;
--직급별 최소 월급이 5000 이상인 직급의 사람들 중 
--부서번호가 50번 이상인 사람들만 출력하시오
--employees , jobs , departments
--사원번호 , 사원명 , 월급 , 직급번호 , 직급별 최소월급 , 부서번호 , 부서명
select * from employees;

select employee_id , emp_name , salary , a.job_id , min_salary , a.department_id 
from employees a, jobs b
and min_salary >=5000; --실패
...
and min_salary >= 5000
order by min_salary asc --성공
;
select * from jobs;
select * from employees;
-- sa_man 직급 sellmanager 사원번호, 사원이름 , 직급번호 , 사원명 , 부서번호 , 부서명  출력
--직급을 가진 where
...
--non equi join stuscore avg 값을 가지고
-- stunum, name , stuinfo.grade , total , avg 값을 출력하시오
select * from stuscore; --a
select * from stuinfo; --b
select * from stugrade; --c


select a.stunum ,c. grade as 학년 , total ,  avg , b.grade as 성적
...
from kor_loan_status
group by period ;
--부서별 총월급 구하기 group by 로 사용하기 연도별로 합산
SELECT SUBSTR(period, 1, 4) AS 연도, SUM(loan_jan_amt)
FROM kor_loan_status
GROUP BY SUBSTR(period, 1, 4);

--부서별 총월급 구하기 group by 로 사용하기
select a.department_id,department_name,sum(salary)
from employees a, departments b
where a.department_id = b.department_id
group by a.department_id , department_name;
--부서별 총월급 구하기 group by 로 사용하기 월별로 합산
SELECT SUBSTR(period, 5, 2) AS 월, SUM(loan_jan_amt)
FROM kor_loan_status
GROUP BY SUBSTR(period, 5, 2);

select * from employees;
select employee_id , emp_name,manager_id,department_id 
from employees
...
select a.employee_id , a.emp_name , a.manager_id , a.emp_name
from employees a, employees b
where a.manager_id = b.employee_id
...
from employees a, employees b , department c , jobs d
where a.manager_id = b.employee_id
and a.department_id = c.department_id
