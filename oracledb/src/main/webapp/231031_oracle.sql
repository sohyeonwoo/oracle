...
select max(salary),min(salay) from employees;
...
select department_id, max(salary),min(salary),round(avg(salary),2)"평균 5000이상" from employees
group by department_id
...
select employee_id , emp_name , department_id , max_salary , min_salary from employees a, jobs b
